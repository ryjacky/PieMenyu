...
class _PieMenuViewState extends State<PieMenuView> {
  ...
  Widget build(BuildContext context) {
    _pieMenu = context.watch<PieMenuController>().pieMenu;
    _pieItems = context.select<PieMenuController, List<PieItem>>(
            (value) => value.pieItems);

    updateParameterThenSetState();
    final currentPieMenuId = context.select<PieMenuController, int>(
          ...
          for (int i = 0; i < _pieItems.length; i++)
            Positioned(
              ...
              child: GestureDetector(
                onTap: () {
                  context.read<PieMenuController>().currentPieItemId =
